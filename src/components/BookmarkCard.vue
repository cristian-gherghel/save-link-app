<template>
  <li
    ...
    <header class="card-header w100 flex flex-between">
      <i class="copy-icon"
         @click="handleCopy"
         v-html="baseIcons.copyLink"
         v-if="showActionBtn" />

      <button v-show="type === 'xs' || showActionBtn"
              class="card-menu m-left-auto"
              aria-label="Options"
              @click.stop="toggleMenu">
        <i :class="{ isLightTheme: theme === 'light' }"
           class="close-icon"
           v-if="isOpen"
           v-html="baseIcons.closeIcon" />

        <span v-else class="dots-container"
              :class="{ 'show-dots': showActionBtn }">
          <i
              v-for="n in 3"
              :key="n"
